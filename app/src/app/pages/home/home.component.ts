...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  async search(searchName: string) {
    ...
    if (this.searchBoard === SearchBoard.minted) {
      ...
      try {
        const metaDataUrl = await this.contractService.getMetadataUrl(this.searchInfo.standardName);
        const url = metaDataUrl.replace('https://metadata.oland.info/oland/', '/img/')

        const imgUrl = await this.httpService.getMetadata(url)
        this.searchInfo.img = imgUrl.image
      } catch (error) {
        this.searchInfo.img = (await this.contractService.getMetadataUrl(this.searchInfo.standardName)).replace('data.json', 'image.png').replace('https://metadata.oland.info/oland/', 'https://test.metadata.oland.info/oland/');
      }
